...
```rust
let count = container.read(count);
let count_plus_one = container.read(count_plus_one);
```
...
- Containers are lock free and are *highly* readable, with writes blocked by a `Mutex`
  - I hit >20 million *reads* per second on my laptop, and that's *per core* (scaling linearly)
  - I hit a limit of ~1.3 million *writes* per second on my laptop (capsule data updates)
...
<CodeGroup title="Temporarily Listening to Capsules">
  ```dart
  final container = CapsuleContainer();
  final handle = container.listen((use) => print(use(fooBarCapsule)));
  ...
  drop(handle); // or just let the handle be dropped at the end of its scope
  ```
</CodeGroup>
...
<CodeGroup title="Indefinitely Listening to Capsules">
  ...
  void listener(CapsuleHandle use) {
    // Register a no-op side effect (you can use any side effect you like here)
    use.register((_) {});
  ...
  fn listener(CapsuleHandle { mut get, register }: CapsuleHandle) {
      register(()); // register (), the no-op side effect
