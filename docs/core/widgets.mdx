...
@rearchWidget
Widget myAppBody(CapsuleHandle use) {
  return Unnest()
...
are automatically passed onto the generated widget's constructor, similar to `@rearchFactory`.
...
Just don't include the `CapsuleHandle` parameter!
